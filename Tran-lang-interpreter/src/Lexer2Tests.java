...
public class Lexer2Tests {
    ...
    public void MultiLineQuotedStringLexerTest() {
        ...
        try {
            ...
            Assertions.assertEquals(Token.TokenTypes.NUMBER, res.get(3).getType());
            Assertions.assertEquals("1.2", res.get(3).getValue());
            Assertions.assertEquals(Token.TokenTypes.QUOTEDSTRING, res.get(4).getType());
            Assertions.assertEquals("Hello\n World", res.get(4).getValue());
    ...
    public void QuotedCharacterLexerTest() {
        ...
        try {
            ...
            Assertions.assertEquals("h", res.get(1).getValue());
    ...
    @Test
    public void IndentTestwithBlackSlashN() {
        var l = new Lexer(
                "loop keepGoing\n" +
                        "    if n >= 15\n" +
                        "        keepGoing = false\n"
    ...
    @Test
    public void IndentTestwithBlackSlashN2() {
        var l = new Lexer(
                "loop keepGoing\n" +
                        "\tif n >= 15\n" +
                        "\t\tkeepGoing = false\n"
    ...
    @Test
    public void ProfessorsIndentstuff() {
        var l = new Lexer(
    ...
    public void IndentTest() {
        ...
        try {
            var res = l.Lex();
            Assertions.assertEquals(18, res.size());
    ...
    public void CommentsTest() {
        var l = new Lexer("if {dhaoihdaoihdoiahdoiah\noiahdoiahdoiahdiao}");
        try {
            var res = l.Lex();
    ...
    @Test
    public void ComplexCommentsTest() {
        var l = new Lexer("if{dh\noo}");
        try {
            var res = l.Lex();
            Assertions.assertEquals(1, res.size());
            Assertions.assertEquals(Token.TokenTypes.IF, res.get(0).getType());
    ...
    @Test
    public void ComplexCommentsTest2() {
        var l = new Lexer("if {dh\noo}");
        try {
            var res = l.Lex();
            Assertions.assertEquals(1, res.size());
    ...
    @Test
    public void errorfixdeletelater() {
        var l = new Lexer("\t\ty = \"\"\n");
        try {
            var res = l.Lex();
            Assertions.assertEquals(4, res.size());
    ...
    public void TestIt() {
        ...
        try {
            var res = l.Lex();
//            Assertions.assertEquals("updateClock",res.get(128).getValue()); //128
            Assertions.assertEquals("updateClock",res.get(134).getValue()); //128
...
//TODO increment linenumber when there is a \n in comment or quoted and singlequoted strings
//TODO remove newline from comment and string stuff
